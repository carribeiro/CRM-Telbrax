...
class DateClass
{
	...
	{	
		if ($dateTime) $this->SetDate($DateTime);
		$time = mktime($this->Hours(), $this->Minutes(), $this->Seconds(), $this->Month(), 1, $this->Year());
		$dt = new DateClass($time);				
		return $dt;
	} //BOM()
	...
	{
		if ($dateTime) $this->SetDate($DateTime);
		$time = mktime($this->Hours(), $this->Minutes(), $this->Seconds(), $this->Month(), date('t', $this->TimeStamp()), $this->Year());
		$dt = new DateClass($time);
		return $dt;
	} //EOM()
